...
const inputStyle = {
  ...
  border: '1px solid #e8ecf4', fontSize: 15, color: '#0d1b2e',
  ...
  return (
    <div>
      ...
      <div style={{ paddingTop: 76 }}>
        <div style={{ background: 'linear-gradient(135deg, #070e1c, #0d1b2e)', padding: '64px 0', textAlign: 'center' }}>
              ...
              <div>
                {[
                  ...
                  <div key={i} style={{ display: 'flex', gap: 16, marginBottom: 28 }}>
                    <div style={{ width: 48, height: 48, borderRadius: 12, background: 'rgba(245,166,35,0.1)', border: '1px solid rgba(245,166,35,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    ...
                    <div>
                      ...
                      <a href={item.href} style={{ fontSize: 15, fontWeight: 600, color: '#0d1b2e' }}>{item.value}</a>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ background: '#fff', borderRadius: 20, padding: 40, border: '1px solid #e8ecf4' }}>
                {sent ? (
                  <div style={{ textAlign: 'center', padding: '32px 0' }}>
                    ...
                    <h3 style={{ fontFamily: 'Space Grotesk', color: '#0d1b2e', marginBottom: 10 }}>Message Sent!</h3>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                      {[{name:'name',label:'Name *',ph:'Your name'},{name:'email',label:'Email *',ph:'your@email.com',type:'email'},{name:'phone',label:'Phone',ph:'+1 000 000 0000'},{name:'subject',label:'Subject',ph:'How can we help?'}].map(f=>(
                        <div key={f.name}>
                          ...
                          <input type={f.type||'text'} name={f.name} value={form[f.name]} onChange={e=>setForm({...form,[e.target.name]:e.target.value})} placeholder={f.ph} style={inputStyle} required={f.name==='name'||f.name==='email'} onFocus={e=>e.target.style.borderColor='#f5a623'} onBlur={e=>e.target.style.borderColor='#e8ecf4'} />
                        </div>
                      ))}
                    </div>
                    <div style={{ marginBottom: 24 }}>
                      <label style={{fontSize:13,fontWeight:600,color:'#374151',marginBottom:6,display:'block'}}>Message *</label>
                      <textarea name="message" value={form.message} onChange={e=>setForm({...form,message:e.target.value})} rows={5} placeholder="Tell us about your project or question..." required style={{...inputStyle,resize:'vertical'}} onFocus={e=>e.target.style.borderColor='#f5a623'} onBlur={e=>e.target.style.borderColor='#e8ecf4'} />
                    </div>
