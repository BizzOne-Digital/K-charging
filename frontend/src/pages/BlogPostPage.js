  ...
  return (
    <div>
      ...
      <div style={{ paddingTop: 76 }}>
        ...
        {!post.coverImage && <div style={{ background: 'linear-gradient(135deg, #052414, #0b3d24)', height: 280 }} />}
        <section className="section reveal">
          <div className="container" style={{ maxWidth: 780 }}>
            ...
            <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
              <span style={{ padding: '4px 12px', borderRadius: 100, background: 'rgba(26,125,60,0.1)', color: '#1a7d3c', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 5 }}><FiTag size={11} />{post.category}</span>
            ...
            <h1 style={{ fontFamily: 'Space Grotesk', fontSize: 'clamp(24px,4vw,38px)', color: '#0b3d24', lineHeight: 1.25, marginBottom: 24 }}>{post.title}</h1>
            {post.author && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 36, paddingBottom: 28, borderBottom: '1px solid #e8ecf4' }}>
                <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(135deg, #0b3d24, #1a7d3c)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700 }}>{post.author.name?.charAt(0)}</div>
                <div><div style={{ fontSize: 14, fontWeight: 600, color: '#0b3d24' }}>{post.author.name}</div><div style={{ fontSize: 12, color: '#94a3b8' }}>K Charging Solutions</div></div>
              </div>
            )}
            ...
            <div style={{ marginTop: 48, padding: 32, background: 'linear-gradient(135deg, #0b3d24, #135c34)', borderRadius: 16, textAlign: 'center' }}>
      ...
      <Footer />
      <style>{`.blog-content h2{font-family:'Space Grotesk',sans-serif;font-size:24px;color:#0b3d24;margin:32px 0 16px}.blog-content h3{font-size:20px;color:#0b3d24;margin:24px 0 12px;font-weight:700}.blog-content p{margin-bottom:20px}.blog-content ul,.blog-content ol{margin:0 0 20px 24px}.blog-content li{margin-bottom:8px}.blog-content strong{font-weight:700;color:#0b3d24}.blog-content a{color:#2f9e55;text-decoration:underline}`}</style>
    </div>
  );
}
