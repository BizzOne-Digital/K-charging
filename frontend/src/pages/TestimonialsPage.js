      ...
      <div style={{ paddingTop: 76 }}>
        ...
        <section className="section reveal" style={{ background: '#f8f9fc' }}>
          <div className="container">
                ...
                {testimonials.map(t => (
                  <div key={t._id} style={{ background: '#fff', borderRadius: 16, padding: 32, border: '1px solid #e8ecf4', boxShadow: '0 2px 12px rgba(0,0,0,0.04)', transition: 'all 0.3s' }}
                    ...
                    <div style={{ display: 'flex', gap: 3, marginBottom: 16 }}>
                      {[...Array(5)].map((_, i) => <FiStar key={i} size={16} fill={i < (t.rating || 5) ? '#1a7d3c' : 'transparent'} color={i < (t.rating || 5) ? '#1a7d3c' : '#e8ecf4'} />)}
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                      {t.avatar ? <img src={t.avatar} alt={t.name} style={{ width: 48, height: 48, borderRadius: '50%', objectFit: 'cover' }} />
                        : <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'linear-gradient(135deg, #0b3d24, #1a7d3c)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 18 }}>{t.name.charAt(0)}</div>}
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 15, color: '#0b3d24' }}>{t.name}</div>
            ...
            <div style={{ marginTop: 64, background: '#0b3d24', borderRadius: 20, padding: '48px 40px', textAlign: 'center' }}>
